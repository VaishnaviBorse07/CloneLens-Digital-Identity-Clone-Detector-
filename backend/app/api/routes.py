...
@router.get("/model/info/image", tags=["ML Models"])
async def get_image_model_info():
    """Returns architecture specs, parameters count, and loaded checkpoint status for the Custom CNN."""
    num_params = sum(p.numel() for p in image_engine.model.parameters()) if image_engine.model else 0
    trainable_params = sum(p.numel() for p in image_engine.model.parameters() if p.requires_grad) if image_engine.model else 0
    
    report_path = "data/images/test_evaluation_report.json"
    cached_report = None
    ...
    return {
        "model_name": "CloneLens Custom Residual CNN (CloneLensCNN)",
        "architecture_summary": "4-Stage Progressive Residual ConvNet with Squeeze-and-Excitation (SE) Channel Attention and Dual Global Pooling",
        "input_resolution": f"{settings.IMAGE_INPUT_SIZE}x{settings.IMAGE_INPUT_SIZE} RGB",
        "total_parameters": num_params,
        "trainable_parameters": trainable_params,
        "device": str(image_engine.device),
        "weights_path": settings.IMAGE_MODEL_WEIGHTS_PATH,
        "weights_loaded": image_engine.weights_loaded,
        "status": image_engine.model_status,
        "explainability": "Grad-CAM (Gradient-weighted Class Activation Mapping) Heatmap Generation",
        "latest_test_benchmark": cached_report
    }


@router.post("/model/evaluate/image", tags=["ML Models"])
async def run_image_model_evaluation(split: str = "test", limit: Optional[int] = 200):
    """Executes on-demand evaluation of the Custom CNN against the test or validation dataset."""
    from backend.ml.image_model.evaluate import evaluate_model
    try:
        results = evaluate_model(
            model_path=settings.IMAGE_MODEL_WEIGHTS_PATH,
            split=split,
            limit_samples=limit
        )
        return results
